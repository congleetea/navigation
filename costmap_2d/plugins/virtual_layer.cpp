...
void VirtualLayer::onInitialize()
{
  ...
  if (virtual_wall_sub_.getTopic() != ros::names::resolve(virtual_wall_topic))
  {
    // we'll subscribe to the latched topic that the map server uses
    ROS_INFO("Requesting the virtual wall...");
    virtual_wall_sub_ = g_nh.subscribe(virtual_wall_topic, 1, &VirtualLayer::IncommingVirtualWall, this);
    map_received_ = false;
    ...
    if (subscribe_to_updates_)
    {
      ...
      map_update_sub_ = g_nh.subscribe(virtual_wall_topic + "_updates", 10, &VirtualLayer::incomingUpdate, this);
...
void VirtualLayer::IncommingVirtualWall(const nav_msgs::OccupancyGridConstPtr& new_map)
{
...
void VirtualLayer::incomingUpdate(const map_msgs::OccupancyGridUpdateConstPtr& update)
{
  unsigned int di = 0;
